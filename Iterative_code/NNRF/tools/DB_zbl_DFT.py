...
import re
import json
from NNautomated.lammpslib import write_lammps_data
...
def zbl_input_file_old(filename, elements):
	e_string = " ".join(elements)
...
pair_style      zbl 3.5 4.5
"""
		string += "pair_coeff 1 1 41 41\n"
		string += "pair_coeff 1 2 41 42\n"
		string += "pair_coeff 1 3 41 73\n"
		string += "pair_coeff 1 4 41 74\n"
		string += "pair_coeff 2 2 42 42\n"
		string += "pair_coeff 2 3 42 73\n"
		string += "pair_coeff 2 4 42 74\n"
		string += "pair_coeff 3 3 73 73\n"
		string += "pair_coeff 3 4 73 74\n"
		string += "pair_coeff 4 4 74 74\n"
		string += """thermo_style    custom step pe pxx pyy pzz pxy pxz pyz
thermo   1
...
write_en = open('energy_tmp.txt','w')
...
for i in range(len(images)):
	...
	write_en.write('%12.4f %12.4f %12.4f \n' % (vol, dft_en-dft_ref, ref_en-ref_ref))
	en = en * (kcal/mol/eV)
	#print(dft_en-dft_ref, ref_en-ref_ref)
	factor = (kcal/mol/eV)
...
write_en.close()
call('rm lmp_* *.lmp *.data force.dump lmp.out min.xyz', shell=True)
