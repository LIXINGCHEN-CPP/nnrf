...
def lammps(lmp_filename,lmp_path, ncores):
	...
	lammps_command =lmp_path + " -var filename " + lmp_filename + " -in base.in > lmp.out"
...
def ref_script_DFT_data(dataname, db_dir, script, element_short, lmp_path, atomsk_path):
	os.chdir(db_dir)
	images = data2images(dataname)
	new_dataname = dataname[:-5] + '_ref.data'
	new_images = []
	...
	if not os.path.exists(new_dataname):
		...
		for i in range(len(images)):
			...
			#zbl_input_file('base.in',element_ordered)
			if str_type == 'slab':
				subprocess.call('cp '+script['slab']+' ./base.in',shell=True)
			else:
				subprocess.call('cp '+script['bulk']+' ./base.in',shell=True)
			...
			if cell_la[3] != 90 or cell_la[4] != 90 or cell_la[5] != 90:
				write_lammps_data(filename=xyz_filename, atoms=atoms, atom_types=atom_types, units='real',triclinic=True)
			else:
				write_lammps_data(filename=xyz_filename, atoms=atoms, atom_types=atom_types, units='real',triclinic=False)
			#atomsk(basename, atomsk_path, xyz_filename)
			lammps(basename, lmp_path, 1)
			...
			#DFT - REAX
			en = (dft_en-dft_ref) - (ref_en-ref_ref)
			print("DFT energy:%12.4f, Ref-pot energy:%12.4f" % (dft_en,ref_en))
			factor = (kcal/mol/eV)
			en = en * factor
			#press = np.array(dft_press) - np.array(ref_press)
			force = [np.array(dft_force[i])*factor-np.array(ref_force[i])*factor for i in range(len(dft_force))]
