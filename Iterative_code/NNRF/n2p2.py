...
# 	#training_dir = base_dir + '/training_' + str(base_name) + subdir
...
# 		#prv_weights_path = base_dir+'/training_'+str(int(base_name)-1)+subdir+'/nnp-data/weights.*.data'
...
def n2p2_ANN_training_nitramine(base_name, base_dir, db_dir, nnp_input_dir, n2p2_path, ncores, queue, walltime, use_old, scheduler, modules):
    """
    Setup and run n2p2 training for nitramine systems
    """
    training_dir = os.path.join(base_dir, '2_training', f'training_{base_name}')
    ...
    # 复制必要的输入文件
    shutil.copy2(os.path.join(db_dir, 'input.data'), '.')
    shutil.copy2(os.path.join(nnp_input_dir, 'input.nn'), '.')
    
    print("Files in working directory:")
    print(subprocess.run(['ls', '-la'], capture_output=True, text=True).stdout)

    # 创建nnp-data目录
    os.makedirs('nnp-data', exist_ok=True)

    # 设置命令
    commands = [
        # 1. 运行nnp-norm
        f'{n2p2_path}/bin/nnp-norm > norm.log 2>&1',
        
        # 2. 运行nnp-scaling (添加nbins参数)
        f'{n2p2_path}/bin/nnp-scaling 100 > scaling.log 2>&1',  # 使用100个bins
        
        # 3. 运行nnp-train
        f'{n2p2_path}/bin/nnp-train > train.log 2>&1'
    ]
    ...
    for i, cmd in enumerate(commands, 1):
        ...
        try:
            ...
            # 检查日志文件
            log_file = cmd.split('>')[-1].strip().split()[0]
            ...
            # 显示所有日志文件
            for log in ['norm.log', 'scaling.log', 'train.log']:
    ...
    # 收集训练结果
    nnp_data_dir = os.path.join(training_dir, 'nnp-data')
    for file in ['input.nn', 'scaling.data']:
        if os.path.exists(file):
            shutil.copy2(file, nnp_data_dir)
        else:
            print(f"Warning: {file} not found")
    
    # 复制最新的权重文件
    weight_files = sorted(glob.glob('weights.*.out'))
    if weight_files:
        latest_weights = weight_files[-1]
        shutil.copy2(latest_weights, nnp_data_dir)
    else:
...
def n2p2_ANN_training(base_name,
	...
	try:
		...
		job_submit_file(path=training_dir,
						...
						walltime=walltime,
						queue=queue,
						commands=['mpiexec -np %s %s' % (str(ncores),norm),
						          'cp -f output.nn input.nn',
								  'mpiexec -np %s %s' % (str(ncores),scaling),
								  'mpiexec -np %s %s' % (str(ncores),train)],
						jobname='Training',
						modules=modules,
						scheduler='slurm')
