...
def convert_cell(ase_cell):
    ...
    """
    #print(np.array(ase_cell))
    cell = np.matrix.transpose(np.array(ase_cell))
...
def write_lammps_data(filename, atoms, atom_types, comment=None, cutoff=None,
                      molecule_ids=None, charges=None, units='metal', triclinic=True):

    elements = list(atom_types.keys())
    element_sorted = list(sorted(elements, key=lambda e: atomic_numbers[e]))
    atom_types2 = {}
    for a, e in enumerate(element_sorted):
        atom_types2[e] = a+1
    ...
    """
	#test triclinc cell
    celltest = atoms.get_cell_lengths_and_angles()
    cell_length = celltest[:3]
    cell_angle = celltest[3:]
    cell_angle_rd = np.deg2rad(cell_angle)
    lx = cell_length[0]
    xy = cell_length[1]*np.cos(cell_angle_rd[2])
    xz = cell_length[2]*np.cos(cell_angle_rd[1])
    ly = np.sqrt(cell_length[1]**2 - xy**2)
    yz = (ly*cell_length[2]*np.cos(cell_angle_rd[0]) - xy*xz) / ly
    lz = np.sqrt(cell_length[2]**2- xy**2 - yz**2)
    ...
    for sym, typ in zip(atom_types2.keys(), atom_types2.values()):
        #print(atom_types2[sym], sym_mass[sym])
        fh.write('{0} {1}\n'.format(atom_types2[sym], sym_mass[sym]))

    #for (sym, typ) in sorted(atom_types2.items(), key=operator.itemgetter(1)):
    #    fh.write('{0} {1}\n'.format(typ, sym_mass[sym]))
    pos = atoms.get_positions() / unit_convert("distance", 'real')
    ...
    for i, (sym, mol, q, pos) in enumerate(
            zip(symbols, molecule_ids, charges, pos)):
        typ = atom_types2[sym]
        #fh.write('{0} {1} {2} {3:16.8e} {4:16.8e} {5:16.8e} {6:16.8e}\n'
        #         .format(i + 1, mol, typ, q, pos[0], pos[1], pos[2]))
        fh.write('{0} {1} {2:16.8e} {3:16.8e} {4:16.8e} {5:16.8e}\n'
                  .format(i + 1, typ, q, pos[0], pos[1], pos[2]))
    if isinstance(filename, basestring):
        fh.close()
