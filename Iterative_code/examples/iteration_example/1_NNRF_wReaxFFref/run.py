...
n2p2_path = "/workspace/CabanaMD-Setup/n2p2"
lmp_path = "/workspace/CabanaMD-Setup/lammps/build/lmp"
...
env = {
    'PATH': [
        '/workspace/CabanaMD-Setup/n2p2/bin',
        '/workspace/CabanaMD-Setup/lammps/build',
        '/usr/local/bin'
    ],
    'LD_LIBRARY_PATH': [
...
Model = NNRF.Iterative_Train.NNRF_wReaxFFref(
             base_dir=base_path,
...
for n in range(1,11):
	...
	Model.vasp_DFT_Species(iteration=n,
							interval=1000,
							species_remote = {'cores':12,
										 'queue':'lpl',
										 'walltime':'24:00:00',
										 'modules':['intel/17.0.1.132', 'impi/2017.1.132'], #Brown/Halstead
										 #'modules':['intel/19.0.5.281','openmpi/3.1.4']
										 },
							reaxff=base_path+'/input_db/ffield_2014',
							ffoption='',
							ReaxFFBO=True,
							)
	...
	Model.DB_ref(iteration=n,reaxff_ref=base_path+'/input_db/ffield')
